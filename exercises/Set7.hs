...
add x (Set xs) = Set $ sort $ nub $ (++) xs [x]
...
data State = Start | EggsAdded | SugarLeft | FlourLeft | MixReady | BakeReady | Error | Finished
  deriving (Eq,Show)
...
step EggsAdded AddFlour = SugarLeft
step EggsAdded AddSugar = FlourLeft

step SugarLeft AddSugar = MixReady
step FlourLeft AddFlour = MixReady

step MixReady  Mix      = BakeReady

step BakeReady Bake     = Finished
...
instance Ord a => Semigroup (Set a) where
  (<>) (Set xs) (Set ys) = foldr add emptySet (xs ++ ys)
...
passwordAllowed pass (MinimumLength len) = length pass >= len
passwordAllowed pass (ContainsSome requiredChars) = not $ null $ intersect pass requiredChars
passwordAllowed pass (DoesNotContain blockedChars) = null $ intersect pass blockedChars
passwordAllowed pass (And req1 req2) = passwordAllowed pass req1 && passwordAllowed pass req2
passwordAllowed pass (Or req1 req2) = passwordAllowed pass req1 || passwordAllowed pass req2
...
operation "*" a1 a2 = Multiplication a1 a2
