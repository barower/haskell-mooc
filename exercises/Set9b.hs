...
prettyPrint size coords = [if j /= size+1 then check (i,j) else '\n' | i <- [1..size], j <- [1..size+1]]
        where check (r,c) = if elem (r,c) coords then 'Q' else '.'
...
prettyPrint2 size coords = [if j /= size+1 then check (i,j) else '\n' | i <- [1..size], j <- [1..size+1]]
        where check (r,c)
                | elem (r,c) coords = 'Q'
                | danger (r,c) coords = '#'
                | otherwise = '.'
...
fixFirst n ((fr,fc):queens)
        | fr > n || fc > n = Nothing
        | not $ danger (fr,fc) queens = Just ((fr,fc):queens)
        | otherwise = fixFirst n ((fr,fc+1):queens)
