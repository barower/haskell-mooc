...
isOrdered (Node val left right) = checkLeft left && checkRight right
        where checkLeft  Empty = True
              checkLeft  (Node leftval  _ _) = if leftval >= val then False else isOrdered left
              checkRight Empty = True
              checkRight (Node rightval _ _) = if rightval <= val then False else isOrdered right
...
set (StepL:steps) newVal (Node oldVal left right) = Node oldVal (set steps newVal left) right
set (StepR:steps) newVal (Node oldVal left right) = Node oldVal left (set steps newVal right)
