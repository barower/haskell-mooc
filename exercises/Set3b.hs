...
sums :: Int -> [Int]
sums lim = sums' 1 0 lim

sums' :: Int -> Int -> Int -> [Int]
sums' i ps lim
        | i == lim   = [newps]
        | otherwise = newps : sums' (i+1) newps lim
        where newps = ps + i
...
sumsOf' :: Int -> [Int] -> [Int]
sumsOf' _   [] = []
sumsOf' sum (x:xs) = newsum : sumsOf' newsum xs
        where newsum = sum + x
...
mymaximum bigger biggest (x:xs)
        | bigger x biggest = mymaximum bigger x       xs
        | otherwise        = mymaximum bigger biggest xs
